...
def ray_trace(ray_origin, ray_direction, obj_loader, lights, camera_position,hdr_env_map, depth=0, max_depth=3, intersection_count=None):
    if depth > max_depth:
...
def ray_trace_path(ray_origin, ray_direction, obj_loader, lights, camera_position, hdr_env_map, depth=0, max_depth=5,intersection_count=None):
    if depth > max_depth:
        return np.array([0, 0, 0])  # Base case for recursion (e.g., for reflections)
    ...
    if closest_hit is None:
        # Sample the environment map if no hit
        return sample_hdr_environment(ray_direction, hdr_env_map)
    else:
        normal = np.cross(closest_triangle[1] - closest_triangle[0], closest_triangle[2] - closest_triangle[0])
        normal = normalize(normal)  # Normalize the normal vector
        ...
        direct_light_color = calculate_lighting(closest_hit, normal, closest_material, lights, camera_position)

        # Russian Roulette for recursion termination
        if np.random.rand() < 0.8 or depth < 3:
            # Reflect the ray in a random direction based on the material
            new_ray_direction = random_reflect(ray_direction, normal)
            indirect_light_color = ray_trace_path(closest_hit, new_ray_direction, obj_loader, lights, camera_position, hdr_env_map, depth + 1, max_depth, intersection_count)

            return direct_light_color + indirect_light_color * 0.8
        else:
            return direct_light_color

def random_reflect(ray_direction, normal):
    # This function generates a new ray direction based on the material properties
    # For simplicity, we'll assume a Lambertian (perfectly diffuse) material
    random_direction = normalize(np.random.randn(3))
    if np.dot(random_direction, normal) < 0:
        random_direction = -random_direction
    return normalize(ray_direction + random_direction)
...
def create_test_scene_ray(obj_loader, camera, lights, hdr_env_map, args):
    intersection_count = [0] if args.intersections else None
    start_time = time.time() if args.rendertime else None
    ...
    for y in range(camera.height):
        for x in range(camera.width):
            ...
            color = ray_trace(ray_origin, ray_direction, obj_loader, lights, camera.position,hdr_env_map,0, 3, intersection_count)
    ...
    if args.intersections:
        print(f"Total Intersections with Triangles: {intersection_count[0]}")

    if args.rendertime:
        end_time = time.time()
        print(f"Render Time: {end_time - start_time} seconds")

def create_test_scene_path(obj_loader, camera, lights, hdr_env_map, args, samples_per_pixel=4):
    intersection_count = [0] if args.intersections else None
    start_time = time.time() if args.rendertime else None
    image = Image.new('RGB', (camera.width, camera.height))
    pixels = image.load()
    ...
    for y in range(camera.height):
        for x in range(camera.width):
            ...
            for _ in range(samples_per_pixel):
                ray_origin, ray_direction = camera.get_ray(x, y)
                color += ray_trace_path(ray_origin, ray_direction, obj_loader, lights, camera.position, hdr_env_map, 0,5, intersection_count)
    ...
    brighter_image.save('rendered_brighter.png')
    render_image = cv2.imread('render.png')
    denoised_image = cv2.fastNlMeansDenoisingColored(render_image, None, 10, 10, 7, 21)
    cv2.imwrite('rendered_denoised.png', denoised_image)
...
def parse_arguments():
    ...
    parser.add_argument('-obj', '--obj', required=True, help='Path to the OBJ file')
    parser.add_argument('-mtl', '--mtl', required=True, help='Path to the MTL file')
    parser.add_argument('-hdr', '--hdr', required=True, help='Path to the HDR image file')
    parser.add_argument('-mode', '--mode', choices=['PT', 'RT'], required=True, help='Mode: PT for Path Tracing, RT for Ray Tracing')
    parser.add_argument('-i', '--intersections', action='store_true',help='Print the number of intersections with triangles')
    parser.add_argument('-s', '--samples', type=int, default=4, help='Number of samples per pixel (only for PT)')
    parser.add_argument('-rt', '--rendertime', action='store_true', help='Display the render time')
...
#    obj_file = r"C:\Users\admin\dev\OpenGL\triangle.obj"
#    mtl_file = r"C:\Users\admin\dev\OpenGL\triangle.mtl"
#    hdr_env_map = load_hdr_image(r"C:\Users\admin\dev\OpenGL\clarens_midday_4k.hdr")
    obj_loader = OBJLoader(obj_file, mtl_file)
    hdr_env_map = load_hdr_image(hdr_file)
    camera = Camera(position=[0, 11, 0], look_at=[0, 0, 0], up_vector=[0, 0, 1], fov=90, width=800, height=600)
    lights = [Light(position=[2, 4, 0], color=[1, 1, 1], intensity=5.0)]


    if mode == 'RT':
        create_test_scene_ray(obj_loader, camera, lights, hdr_env_map, args)
    elif mode == 'PT':
        samples_per_pixel = args.samples
        create_test_scene_path(obj_loader, camera, lights, hdr_env_map, args, samples_per_pixel)
